...
def test_ops_route_detects_write_request_but_keeps_plan_boundary():
    ...
    route = route_ops_request(
        "请直接 kill 掉 102_web 或占用 5045 的进程",
        intent=SimpleNamespace(operation="platform_stop"),
    )
...
def test_ops_route_does_not_treat_platform_name_as_port_for_restart_plan():
    ...
    route = route_ops_request(
        ...
        intent=SimpleNamespace(operation="platform_restart"),
    )
...
def test_ops_route_does_not_treat_conditional_failure_stop_as_stop_action():
    ...
    route = route_ops_request(
        ...
        intent=SimpleNamespace(operation="environment_setup"),
    )
...
def test_ops_route_never_promotes_action_words_without_structured_intent():
    from klonet_agent.ops.routing import route_ops_request

    route = route_ops_request("为什么之前会把‘不要重启’识别成重启计划？")

    assert route.action == "inspect"
    assert route.mode == "只读诊断"
    assert "create_ops_operation_plan" not in route.recommended_tools
