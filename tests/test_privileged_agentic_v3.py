...
def test_complete_config_compiler_reads_detailed_expected_changes():
    from klonet_agent.ops.privileged.contracts import (
        PlanResource,
        PrivilegedPlan,
        PrivilegedStep,
    ...
    resources = [
        ...
        *[
            ...
            for role, value in {
                ...
                "web_terminal_port": 47003, "mysql_port": 47004,
                "redis_port": 47005, "rabbitmq_port": 47006,
            }.items()
        ],
    ]
    plan = PrivilegedPlan(
        plan_id="expected-config", goal="deploy isolated Klonet",
        risk="high", resources=resources, steps=[],
    )
    semantic = PrivilegedStep(
        step_id="config",
        title="Configure new instance ports and service endpoints",
        objective="Edit the cloned config for the new instance",
        expected_changes=[
            "Set master_port, worker_port, web_terminal_port",
            "Set master_ip, mysql_ip, rabbitmq_ip to 127.0.0.1",
            "Set mysql_port, redis_port, rabbitmq_port",
        ],
        risk="medium",
    )
    ...
    assert {item.get("attribute") for item in items} >= {
        "master_ip", "mysql_ip", "rabbitmq_ip", "master_port",
        "worker_port", "web_terminal_port", "mysql_port", "redis_port",
        "rabbitmq_port", "celery_redis_port_db", "celery_rabbitmq_port_db",
    }
...
def test_complete_config_compiler_does_not_expand_celery_only_configuration():
    from klonet_agent.ops.privileged.contracts import (
        PlanResource,
        PrivilegedPlan,
        PrivilegedStep,
    ...
    semantic = PrivilegedStep(
        step_id="celery-config",
        title="Configure Celery broker/backend endpoints",
        objective="Set Celery Redis and RabbitMQ endpoint ports with DB suffixes",
        expected_changes=["Celery endpoints change"],
        risk="medium",
    )
