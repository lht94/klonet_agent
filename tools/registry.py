...
TOOLS = [
    ...
    _tool(
        ...
        "为 deploy_platform、restart_platform 或 destroy_platform 创建受控 Ops 操作计划。只保存计划，不执行任何环境修改；可为步骤绑定白名单 recipe_id 和结构化参数，但必须等用户确认后才能执行。",
        {
            ...
            "recipe_bindings": {
                "type": "object",
                "description": "可选：按 step_id 绑定受控 recipe，例如 restart_screen_component、prepare_project_files、extract_archive、run_install_script、write_ops_file。extract_archive 参数为 {\"archive_path\":\"/path/pkg.tar\",\"destination_dir\":\"/root\"}；run_install_script 只允许 base_requ_setup.sh NORMAL 或 docker_service.sh，参数为 {\"script_dir\":\"/root/vemu_install_new_gen\",\"script_name\":\"base_requ_setup.sh\",\"script_args\":\"NORMAL\"}；write_ops_file 参数为 {\"path\":\"/path/config.py\",\"content\":\"...\"}，dry-run 时只脱敏预览，真实执行会备份原文件并拒绝 .env、密钥、token、password 等敏感路径。只保存绑定，不执行。",
            },
        },
        ["operation", "target"],
    ),
