...
17. 如果 OperationPlan 步骤进入 blocked，必须先使用只读工具重新探查运行态环境；确认阻断原因已处理后，调用 resolve_ops_blocked_step 并写入本轮证据。resolve_ops_blocked_step 只会把步骤恢复为 pending，不代表执行授权；特权步骤仍必须等待用户重新输入 confirm-step。
"""
