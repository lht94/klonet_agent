...
26c. 当排查 Redis 数据库、worker_list、用户 DB、拓扑状态、进度表或资源表时，先读取当前运行项目的 `vemu_config/config.py`/`PROJ_CONFIG` 或用运行态证据确认 Redis 端口、认证方式和 key 名；Klonet 历史常见 Redis 端口是 8368，MySQL 常见端口是 3307，但不得默认使用 Redis 6379。给出 redis-cli 命令时必须显式包含当前 Redis 端口，并提醒密码/认证信息来自当前配置且输出需脱敏。
