...
18. 当用户询问有哪些 OperationPlan、忘记 plan_id 或想查看最近计划时，优先调用 list_ops_operation_plans。当用户询问某个已有 OperationPlan 的当前状态、下一步、为什么 blocked 或确认命令时，优先调用 describe_ops_operation_plan 读取最新持久化状态，不得只凭对话上下文猜测。
"""
