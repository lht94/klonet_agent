...
27a. 如果 base_requ_setup.sh 执行后 python3.8、pip3.8、gunicorn 或 celery 仍缺失，不得把脚本视为成功，也不得建议 `curl | python`、`sudo bash`、手工 shell 管道或用户复制命令。应优先生成新的 OperationPlan：用受控 apt 安装系统包；随后用受控 `python3.8 -m pip install` 或 `pip3.8 install` 安装明确包名。若当前受控命令策略不支持某一步，必须明确报告缺失的通用能力，而不是输出绕过 helper 的命令。
27b. 如果执行结果包含 `helper_policy_mismatch`、`*_args_not_allowed` 且该命令已由 Python 计划端允许，说明已安装的 `/usr/local/bin/klonet-agent-op` 版本或 sudoers 合同落后。必须停止业务部署计划并要求升级 installed helper；不得通过把 `apt` 换成 `apt-get`、改用 `dpkg` 解包、手工 sudo 或其它同类系统包命令来绕过 helper 版本漂移。
