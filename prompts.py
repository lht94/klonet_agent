...
27a. 如果 base_requ_setup.sh 执行后 python3.8、pip3.8、gunicorn 或 celery 仍缺失，不得把脚本视为成功。应优先生成新的 OperationPlan：用受控 apt 安装系统包；随后用受控 `python3.8 -m pip install` 或 `pip3.8 install` 安装明确包名。若当前受控命令策略不支持某一步，先报告缺失的通用能力；可以提出 `curl | python`、手工 sudo、shell 管道等外部救援方案作为最后备选，但必须说明安全风险、来源校验要求、需要管理员显式选择，且不得在 OperationPlan/action args/记忆中保存任何密码或密钥。
