...
DEFAULT_OPS_ACTIONS = (
    ...
    OpsActionSpec(
        ...
        confirmation_scope="step",
        category="process",
        description="向明确 PID 发送允许的信号；有 PGID 证据时可用 scope=process_group, pgid=<PGID> 终止同一进程组",
    ...
    OpsActionSpec(
        ...
        confirmation_scope="step",
        category="process",
        description="按已确认 runtime_cwd 和端口证据停止旧 Klonet 运行实例的进程组，并验证端口释放",
        preconditions=("runtime_cwd_observed", "port_owner_evidence_observed"),
        effects=("runtime_process_groups_signalled", "runtime_ports_released"),
        postconditions=("runtime_ports_not_listening",),
        backends=("ops-privilege",),
    ),
    OpsActionSpec(
        "repair_klonet_active_master_ip",
        "_repair_klonet_active_master_ip",
        "dangerous",
        True,
        confirmation_scope="step",
        category="config",
        description="????? Klonet active config class ?? master_ip ??????",
        preconditions=("active_config_observed", "local_ip_observed"),
        effects=("config_file_updated",),
        postconditions=("active_master_ip_matches_local_ip",),
        backends=("ops-privilege",),
    ),
    OpsActionSpec(
        "ensure_klonet_redis_instance",
        "_ensure_klonet_redis_instance",
        "privileged",
        True,
        confirmation_scope="step",
        category="service",
        description="?? Klonet ?????? Redis ??????? Redis ??",
        preconditions=("klonet_config_observed", "redis_port_and_password_observed"),
        effects=("redis_instance_started",),
        postconditions=("redis_port_listening",),
        backends=("ops-privilege",),
    ),
    OpsActionSpec(
