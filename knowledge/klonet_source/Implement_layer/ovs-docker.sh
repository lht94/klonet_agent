...
add_port () {
    ...
    # create veth pair and add portname_c to container
    nsenter --target "$$" --net ip link add "${PORTNAME}_l" type veth peer name "${PORTNAME}_c" netns "$PID"
    ...
    nsenter --target "$PID" --net ip link set dev "${PORTNAME}_c" name "$INTERFACE"
    nsenter --target "$PID" --net ip link set "$INTERFACE" up
    # ip netns exec "$PID" ip link set dev "${PORTNAME}_c" name "$INTERFACE"
    # ip netns exec "$PID" ip link set "$INTERFACE" up

    if [ -n "$MTU" ]; then
        # ip netns exec "$PID" ip link set dev "$INTERFACE" mtu "$MTU"
        nsenter --target "$PID" --net ip link set dev "$INTERFACE" mtu "$MTU"
    fi

    if [ -n "$ADDRESS" ]; then
        # ip netns exec "$PID" ip addr add "$ADDRESS" dev "$INTERFACE"
        nsenter --target "$PID" --net ip addr add "$ADDRESS" dev "$INTERFACE"
    fi

    if [ -n "$MACADDRESS" ]; then
        # ip netns exec "$PID" ip link set dev "$INTERFACE" address "$MACADDRESS"
        nsenter --target "$PID" --net ip link set dev "$INTERFACE" address "$MACADDRESS"
    fi

    if [ -n "$GATEWAY" ]; then
        # ip netns exec "$PID" ip route add default via "$GATEWAY"
        nsenter --target "$PID" --net ip route add default via "$GATEWAY"
    fi
}
