...
from .vm_cmd_execer_ssh import vm_cmd_execer, vm_file_transformer
    ...
    def cp_file(self, static_path='/home/'):
        ...
        if self.NEservice == 'kvm':
            ...
            try:
                ...
                commands = [f'ftp {get_host_ip()}', 'ftp_vm_use', 'vemu',
                            f'put {self.file_path} {static_path}{self.file_name}', 'y',
                            'quit']
                if self.NEtype == 'host':
                    # 虚机启动ftp
                ...
                timeout_s = 999
                time_gap_list = [0.8]*len(commands)
                ...
                '''
                file_transformer = vm_file_transformer(vm_id = self.container_id)
                file_transformer.get_file(self.file_path, static_path+self.file_name)
    ...
    def cp_file(self, container_store_path='/home/'):
        ...
        if self.NEservice == 'kvm':
            ...
            try:
                """
                commands = [f'ftp {get_host_ip()}', 'ftp_vm_use', 'vemu',
                            f'get {self.worker_file_path} {container_store_path}/{self.worker_file_name}',
                            'y', 'quit']
                ...
                file_transformer = vm_file_transformer(vm_id = self.container_id)
                file_transformer.put_file(self.worker_file_path,container_store_path+'/'+self.worker_file_name)
                file_transformer.close()
                return {
                        'code': 1,
                        'msg': 'Upload success!'
