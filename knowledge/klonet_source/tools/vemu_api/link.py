...
class LinkManager(Manager):
    ...
    def dynamic_add_link(self, link_name, src_node, dst_node,vm_sourcePort="",vm_targetPort="", src_IP="",
        dst_IP=""):
        '''动态添加链路
        ...
        payload["info"]["targetType"] = dst_node.type
        payload["info"]["VMsourcePort"] = vm_sourcePort
        payload["info"]["VMtargetPort"] = vm_targetPort

        resp = self._post("/modification/link/", json=payload)
        self._check_resp_code(self._parse_resp(resp))

        # 修改节点信息 这里链路ip配置暂时只支持容器
        if src_IP != "" and src_node.service == "docker":
        ...
        if dst_IP != "" and dst_node.service == "docker":
    ...
    def config_link(self, src_link_config, dst_link_config):
        ...
        # wudx
        # 新办法
        # 采用深拷贝的方式写入payload，来应对一些用户只想初始化一次，但后续一直对同一个对象进行反复修改
        # 如果不这么做，就会导致下面拼接"link_"前缀的操作会越拼越长
        src = copy.deepcopy(src_link_config)
        dst = copy.deepcopy(dst_link_config)
        src.link = f"link_{src.link}"
        dst.link = f"link_{dst.link}"
        
        payload = {
            "user": self.user,
            "topo": self.project,
            "links": [src.dictform(), dst.dictform()]
        }
