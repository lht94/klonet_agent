...
- Ops 新增 `run_ops_command` 作为 OperationPlan action，用于可修改环境的通用结构化命令。它仍然不接受 Shell 字符串，只接受 `program`、`argv` 数组和 `cwd`；当前策略覆盖 make、git clone/pull/push/checkout/submodule/status/rev-parse、apt update/install、cp/install 到系统库目录、insmod/rmmod、tc qdisc add/del/show。系统会按命令分类设置风险：workspace 构建、git clone/pull/checkout 通常 plan confirm 后执行；git push、apt、内核模块、tc 改动和系统目录安装要求 `confirm-step`，需要 root 的命令通过 helper/sudoers 执行并记录输出。
- Ops 允许 root 只读诊断：`read_root_file` 通过 root helper 读取普通权限无法访问的文件；`read_ops_file` 和 `inspect_install_scripts` 在遇到 `/root/...` 权限问题时会自动 fallback 到 helper。这个能力只读、不写入、不执行脚本；真正修改仍必须走 OperationPlan/action。
...
当前项目日志的问题不是没有文件路径，而是没有形成稳定的项目记忆维护机制。系统已经能通过 `user_id + project_id` 定位到 `journals/{user_id}/{project_id}.md`，但这个文件更像一个空档案袋：只有在工具被显式调用时才创建或更新，且 Mentor 模式主要负责回答问题，缺少主动维护项目日志的权限和提示词规则。

一种优化方向是引入专门的 Project Journal Agent，也就是项目日志维护子 Agent。它不负责回答用户问题，只在每轮对话结束后判断是否有内容需要沉淀到项目日志。这样 Mentor Agent 可以专注教学、解释和答疑，项目日志维护则交给后台整理流程。
...
用户
  ↓
Mentor Agent：回答问题、解释概念、引导学习
  ↓ 本轮用户输入 + Mentor 最终回答 + 当前项目日志摘要
Project Journal Agent：判断是否更新项目日志
  ↓ 结构化更新决策
ProjectJournal Store：写入 journals/{user_id}/{project_id}.md
```

这个子 Agent 适合负责：

- 抽取项目目标，例如“研究 Klonet 从单 Worker 升级到多 Worker”。
- 判断项目阶段，例如“正在梳理项目背景与研究意义”。
- 记录关键事实，例如资源调度、拓扑切分、跨 Worker 链路、状态同步、进度聚合等研究重点。
- 记录用户纠正过的重要结论，例如“聚合进度应写入 MySQL，而不是 Redis”。
- 维护下一步，例如继续补充研究意义、技术路线或系统设计。
...
为了避免多 Agent 带来的沟通和事实污染问题，Project Journal Agent 不应设计成自由对话型 Agent，而应设计成受约束的后台整理器。它只输出结构化决策，例如：
...
```json
{
  ...
  "confidence": "high",
  "updates": {
    "goal": "研究 Klonet 从单 Worker 升级到多 Worker，重点分析资源调度、拓扑切分、跨 Worker 链路、状态同步和进度聚合机制。",
    "current_status": "正在梳理项目背景与研究意义",
    "events": [
      "用户确认项目主题与单 Worker 到多 Worker 升级相关"
    ],
    "next_step": "补充研究意义和技术路线"
  },
  "evidence": [
    "用户说：描述从单worker升级到多Worker的研究内容",
    "用户问：做这个项目的背景和意义"
  ]
}
```

主程序再根据这个 JSON 调用安全的 ProjectJournal API 写入日志。这样可以避免子 Agent 直接改文件，也方便做去重、冲突处理和审计。

主要风险与控制方式：

- 沟通成本：不要给子 Agent 全量历史，只给本轮输入、最终回答、少量项目相关历史和当前项目日志摘要。
- 事实污染：只有用户明确表达、工具证据、知识库证据、源码证据才能写入“项目事实”；模型推断应标为“待确认”。
- 重复写入：ProjectJournal 层提供 `append_event_once`、`update_goal_if_empty`、`merge_next_step` 等安全更新方法。
- 冲突处理：用户纠正项目方向时，子 Agent 输出 `correction: true`，日志更新目标并保留修订记录。
- 异步一致性：后台写日志失败时应有 trace 和重试机制，避免用户以为已经记住但实际没有落盘。

最小实现方案：

1. 新增 `journal/maintainer.py`。
2. 定义 `JournalUpdateDecision` 结构。
3. 每轮 Mentor 回答后调用 Journal Maintainer。
4. Maintainer 只返回 JSON，不直接写文件。
5. 主程序根据 JSON 调用 ProjectJournal 的安全更新方法。
6. 增加测试：用户确认“从单 Worker 升级到多 Worker”后，项目日志目标必须写入对应日志文件。

这个方案的核心原则是：可以引入子 Agent，但它应该是受约束的项目日志维护器，而不是另一个自由协作型聊天 Agent。

## 当前 Mentor 的公共文档与源码双库检索流程

当前 Mentor 不把所有内容混进一个知识库，而是根据问题需要选择不同的证据源：

```text
用户问题
   │
   ├─ 概念、使用、部署流程
   │    └─ 模型通常调用 search_knowledge
   │         └─ 公共文档 BM25 + 向量检索
   │
   ├─ 函数、接口、配置项、启动脚本、报错代码
   │    └─ 模型通常调用 search_code
   │         ├─ 先在真实源码中精确搜索
   │         └─ 无精确命中时，检索源码 BM25 + 向量索引
   │
   └─ 排障、调用链、实现原理
        └─ 模型可能同时调用两者
             ├─ 公共文档提供流程和排障思路
...
1. `search_knowledge` 只检索公共文档库，不会在工具内部自动继续检索源码库。
2. `search_code` 的两级检索面对的是同一份源码：先用 ripgrep 做字面量精确搜索；完全没有精确命中时，再使用源码 BM25 和源码向量索引做语义召回。
3. 公共文档和源码可以在同一轮都被使用，但当前是否调用哪个工具仍由 Mentor 模型根据 Prompt 和问题语义判断，不是程序层强制执行的任务路由。

因此，当前流程可以称为传统单库 RAG 的升级形态，更准确的说法是：

- 路由式 RAG：不同任务倾向进入不同证据库。
- 混合检索 RAG：每个语义库内部结合 BM25、精确匹配、向量相似度和 metadata 权重。
- 多源证据 RAG：公共文档解释规范和流程，真实源码证明当前实现。
- 轻量 Agentic RAG：模型可以在 tool loop 中继续搜索源码并读取具体文件。

但它还不是完全确定性的多路 RAG。当前任务类型只会影响模型的工具选择倾向：

- 概念、平台介绍、使用说明和标准部署流程，通常优先查公共文档。
- 函数、API、配置项、启动脚本和具体报错，Prompt 要求优先查源码。
- 故障排查、调用链和实现原理，模型可能同时查询公共文档和源码。

如果要继续升级，应把“是否查公共文档、是否查源码”收敛到程序层的确定性路由：
